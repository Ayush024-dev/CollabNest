...
const Posts = ({ users, OpenCommentSection, Feed, likedMap, fromProfile, onShowError }) => {
    ...
    return (
        <div className="h-[calc(100vh-160px)] w-full">
            <div className="h-full overflow-y-auto overflow-x-hidden custom-scrollbar pr-4 box-border w-full">
                <div className="flex flex-col gap-5 font-inconsolata w-full">
                    {feeds.map((feed) => (
                        <div key={feed._id} className="rounded-md bg-lightBlue p-4 shadow-2xl w-full box-border overflow-hidden">
                            <div className="flex justify-between mb-4 gap-4">
                                        ...
                                        <p className="text-black text-xl font-bold truncate">
                                            {fromProfile ? (
                                                ...
                                                users?.data?.[feed.postId?.toString()]?._id ? (
                                                    <Link href={`/components/UserProfile?user=${users.data[feed.postId]._id}`} className="hover:underline">
                                ...
                                <div className="text-black flex flex-col items-end flex-shrink-0">
                                    <p className='text-center text-sm'>{feed.type}</p>
                                    <p className="text-sm">{feed.field}</p>
                                </div>
                            </div>

                            {/* Post Content with proper text wrapping */}
                            <div className="text-black mb-4 break-words whitespace-pre-wrap overflow-wrap-anywhere w-full">
                                {feed.content}
                            </div>
                                ...
                                <div className="mb-4 w-full">
                                    <div className="relative w-full">
                                        ...
                                        <div className="relative group w-full">
                                            <div className="w-full max-w-full aspect-video overflow-hidden rounded-lg shadow-md bg-gray-100">
                                                <img
                            ...
                            <div className="post_footer w-full mt-3 flex gap-2">
                                <Button
                                    onClick={() => likePosts({ postId: feed._id })}
                                    variant="ghost"
                                    className="text-black hover:bg-gray-200 flex items-center gap-1"
                                >
